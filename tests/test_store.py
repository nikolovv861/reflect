...
def test_search_multi_term_requires_all_words(tmp_path: Path):
    save(Page(day=date(2026, 9, 3), model="m", prompt_version="v1",
              blocks=[Block(WRITING, "The deadline came up at standup.")]), tmp_path)
    save(Page(day=date(2026, 9, 2), model="m", prompt_version="v1",
              blocks=[Block(WRITING, "The deadline again, alone.")]), tmp_path)
    both = search(tmp_path, "deadline standup")
    assert len(both) == 1
    assert both[0].key == "2026-09-03"


def test_search_ranks_denser_matches_first(tmp_path: Path):
    save(Page(day=date(2026, 9, 2), model="m", prompt_version="v1",
              blocks=[Block(WRITING, "focus, and more focus, always focus.")]), tmp_path)
    save(Page(day=date(2026, 9, 3), model="m", prompt_version="v1",
              blocks=[Block(WRITING, "a passing note about focus.")]), tmp_path)
    hits = search(tmp_path, "focus")
    assert hits[0].key == "2026-09-02"  # three mentions outranks one


# --- on this day -----------------------------------------------------------


def test_on_this_day_finds_the_same_date_a_year_ago(tmp_path: Path):
    save(Page(day=date(2025, 9, 6), model="m", prompt_version="v1",
              blocks=[Block(WRITING, "A year ago today.")]), tmp_path)
    echoes = on_this_day(tmp_path, date(2026, 9, 6))
    assert [label for label, _ in echoes] == ["A year ago"]
    assert echoes[0][1].day == date(2025, 9, 6)


def test_on_this_day_finds_a_month_ago(tmp_path: Path):
    save(Page(day=date(2026, 8, 6), model="m", prompt_version="v1",
              blocks=[Block(WRITING, "Last month.")]), tmp_path)
    echoes = on_this_day(tmp_path, date(2026, 9, 6))
    assert ("A month ago", ) == tuple(label for label, _ in echoes)


def test_on_this_day_ignores_today_and_empty_days(tmp_path: Path):
    save(Page(day=date(2026, 9, 6), model="m", prompt_version="v1",
              blocks=[Block(WRITING, "Today itself.")]), tmp_path)
    save(Page(day=date(2025, 9, 6), model="m", prompt_version="v1",
              blocks=[Block(QUESTION, "Only a question, no writing.")]), tmp_path)
    assert on_this_day(tmp_path, date(2026, 9, 6)) == []


# --- practice history ------------------------------------------------------


def test_practice_history_returns_prior_same_practice_days_newest_first(tmp_path: Path):
    for d in (1, 2, 3):
        save(Page(day=date(2026, 9, d), model="m", prompt_version="v1",
                  practice="attention",
                  blocks=[Block(WRITING, f"day {d}")]), tmp_path)
    # A different practice on another day must not leak in.
    save(Page(day=date(2026, 8, 28), model="m", prompt_version="v1",
              practice="free", blocks=[Block(WRITING, "unrelated")]), tmp_path)
    hist = practice_history(tmp_path, "attention", date(2026, 9, 4))
    assert [p.day.day for p in hist] == [3, 2, 1]


def test_practice_history_excludes_today_and_empty_and_respects_limit(tmp_path: Path):
    for d in (1, 2, 3, 4):
        save(Page(day=date(2026, 9, d), model="m", prompt_version="v1",
                  practice="attention",
                  blocks=[Block(WRITING, f"day {d}")]), tmp_path)
    hist = practice_history(tmp_path, "attention", date(2026, 9, 4), limit=2)
    assert [p.day.day for p in hist] == [3, 2]  # not day 4 (today), capped at 2
